...
function notify_clients(player_action){
	...
	switch(player_action) {
		case PLAYER_EVENT.left:
			...
			buffer_write(global.server_buffer, buffer_u8, STATUS.wait_for_other);
		...
		case PLAYER_EVENT.joined:
			if(global.num_player == 1) {
				...
				buffer_write(global.server_buffer, buffer_string, string(ds_map_find_value(con_server.socket_to_instanceid, socket^3).name)); //retrieve socket's object's name
				//show_message("line 41:" + string(ds_map_find_value(con_server.socket_to_instanceid, socket^3).name));
				network_send_packet(socket, global.server_buffer, buffer_tell(global.server_buffer)); //11 ^^ 10 = 01 send to 
			}
			break;
	}
}